...
def cmd_extract(args):
    ...
    if args.anonymize:
        from resume_segmentation.services.pii_anonymizer import PIIAnonymizer

        anonymized = PIIAnonymizer().anonymize(result.profile)
        if args.enrich:
            output["profile"] = anonymized.profile.model_dump()
            output.setdefault("meta", {})["anonymized_fields"] = anonymized.fields_masked
        else:
            output = anonymized.profile.model_dump()
...
def cmd_validate(args):
    ...
    report = result.enrichment.validation_report
    if report is None:
        print("Error: validation report unavailable", file=sys.stderr)
        return 1
    ...
    if args.summary:
        ...
        print(f"  Valid:       {summary['validated_count']}")
        print(f"  Invalid:     {summary['invalid_count']}")
        print(f"  Grey Area:   {summary['grey_area_count']}")
...
def cmd_batch(args):
    ...
    pdfs = sorted(list(input_dir.rglob("*.pdf")) + list(input_dir.rglob("*.docx")))
    if not pdfs:
...
def main():
    ...
    p_extract = sub.add_parser("extract", help="Extract a single PDF or DOCX resume")
    p_extract.add_argument("pdf", help="Path to PDF or DOCX file")
    ...
    p_quality.add_argument("pdf", help="Path to PDF or DOCX file")

    p_validate = sub.add_parser("validate", help="Validate extracted resume structure and ATS readiness")
    p_validate.add_argument("pdf", help="Path to PDF or DOCX file")
    p_validate.add_argument("-o", "--output", help="Output JSON file path")
    p_validate.add_argument("--summary", action="store_true", help="Print a human-readable validation summary")

           
    p_batch = sub.add_parser("batch", help="Batch extract all PDF and DOCX resumes in a directory")
