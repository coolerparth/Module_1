...
from .knowledge_base import load_skill_alias_ontology
from .semantic_skill_matcher import match_skill, normalize_unknown_skill
...
_SKILL_DB: list[SkillEntry] = [
    ...
    SkillEntry("Large Language Models", SkillCategory.ML_AI, 1.0,
               frozenset(["llm", "llms", "large language model", "large language models"])),
    SkillEntry("Generative AI",SkillCategory.ML_AI, 1.0,
               frozenset(["genai", "gen ai", "generative artificial intelligence"])),
    SkillEntry("RAG (Retrieval-Augmented Generation)", SkillCategory.ML_AI, 1.0,
               frozenset(["rag", "retrieval augmented generation", "retrieval-augmented generation"])),
    SkillEntry("MCP",          SkillCategory.ML_AI, 0.8,
               frozenset(["model context protocol"])),
    ...
    SkillEntry("Notion",       SkillCategory.TOOLS, 0.4, frozenset()),
    ...
    SkillEntry("Machine Learning",SkillCategory.ML_AI, 1.0,
               frozenset(["ml", "machine-learning", "machine learning ml", "statistical learning",
                          "supervised learning", "unsupervised learning"])),
    SkillEntry("Deep Learning", SkillCategory.ML_AI, 1.0,
               frozenset(["dl", "deep-learning", "deep learning dl", "neural networks", "ann", "dnn"])),
    SkillEntry("Artificial Intelligence", SkillCategory.ML_AI, 1.0,
               frozenset(["ai", "artificial-intelligence", "artificial intelligence ai", "intelligent systems"])),
    SkillEntry("Data Science", SkillCategory.ML_AI, 1.0,
               frozenset(["data-science", "data analytics", "ds", "data scientist"])),
    SkillEntry("Computer Vision", SkillCategory.ML_AI, 1.0,
               frozenset(["cv", "image recognition", "object detection", "image processing"])),
    SkillEntry("Natural Language Processing", SkillCategory.ML_AI, 1.0,
               frozenset(["nlp", "text mining", "text analytics", "text processing",
...
def _coerce_skill_category(value: object) -> SkillCategory | None:
    if not isinstance(value, str):
        return None
    key = value.strip()
    if not key:
        return None
    if key in SkillCategory.__members__:
        return SkillCategory[key]
    for category in SkillCategory:
        if key.lower() == category.value.lower():
            return category
    return None
...
def _build_lookup() -> dict[str, SkillEntry]:
    ...
    external_aliases = load_skill_alias_ontology()
    for canonical, payload in external_aliases.items():
        if not isinstance(canonical, str):
            continue
        entry = lookup.get(_norm(canonical))
        if not entry and isinstance(payload, dict):
            category = _coerce_skill_category(payload.get("category"))
            if category is not None:
                weight = float(payload.get("weight", 0.8))
                entry = SkillEntry(canonical=canonical, category=category, weight=weight)
                lookup[_norm(canonical)] = entry
        if not entry:
            continue
...
_LEVEL_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"\b(basic|beginner|junior|elementary)\b",             re.I), "Beginner"),
...
_SKILL_PREFIX_RE = re.compile(
    r"^(?:languages?|development|design|databases?|developer tools?|"
    r"business intelligence tools?|ai/ml|iot platforms?|frameworks?|"
    r"libraries|tools?|platforms?|technologies)\s*:\s*",
    re.IGNORECASE,
)
...
def _preclean_skill(raw: str) -> str:
    return _SKILL_PREFIX_RE.sub("", raw.strip())


def _looks_like_skill(raw: str) -> bool:
    lowered = raw.strip().lower()
    if not lowered:
        return False
    if lowered.startswith(_NON_SKILL_PREFIXES):
        return False
    if len(lowered) > 45:
        return False
    if lowered.count(",") >= 2:
...
def normalize_skill(raw: str) -> NormalizedSkill:
    clean, level = _extract_level(_preclean_skill(raw))
    key = _norm(clean)
    ...
    semantic_match = match_skill(clean)
    if semantic_match and semantic_match != clean:
        sem_key = _norm(semantic_match)
        sem_entry = _LOOKUP.get(sem_key)
        if sem_entry:
            return NormalizedSkill(
                raw=raw,
                canonical=sem_entry.canonical,
                category=sem_entry.category,
                weight=sem_entry.weight,
                level=level,
                was_normalized=True,
            )
        return NormalizedSkill(
            raw=raw,
            canonical=semantic_match,
            category=_guess_category(semantic_match),
            weight=0.7,
            level=level,
            was_normalized=True,
        )

    canonical = normalize_unknown_skill(clean)
    return NormalizedSkill(
        raw=raw,
        canonical=canonical,
        category=_guess_category(canonical),
        weight=0.6,
        level=level,
        was_normalized=(canonical != clean),
    )
