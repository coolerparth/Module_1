...
from .spacy_ner_extractor import _get_best_nlp, ner_available
...
_WEAK_OPENINGS: frozenset[str] = frozenset([
    ...
    "was involved", "was responsible", "was tasked", "was assigned",
    "assisted in", "helped in", "collaborated on", "participated in",
    "contributed to", "supported the", "worked alongside",
    "exposure to", "familiar with", "knowledge of", "understanding of",
    "member of", "part of the", "working knowledge",
])
...
_METRIC_RE = re.compile(
    r"\$[\d,.]+"
    r"|\d+[kKmMbB]\+?"
    r"|\d+\s*(?:million|billion|thousand|lakh|lac|crore|cr)\b"
    r"|\d+\s*(?:users|customers|requests|transactions|clients|members)\b"
    r"|\d+\s*(?:lines|repos|servers|teams?|engineers?|countries|markets?)\b"
    r"|\d+\s*(?:tps|rps|qps|rpm|ms|seconds?|minutes?|hours?|days?|weeks?)\b"
    r"|\d+\s*%"
    r"|\d+x\b"
    r"|\d+\s*(?:cr|inr|usd|eur|gbp)\b",
    re.IGNORECASE,
)

_TIME_REDUCTION_RE = re.compile(
    r"(?:from|reduced?|cut|decreased?)\s+\d+\s*(?:hours?|hrs?|minutes?|mins?|days?|seconds?|ms)"
    r"\s+to\s+\d+\s*(?:hours?|hrs?|minutes?|mins?|days?|seconds?|ms)",
    re.IGNORECASE,
)

_INDIAN_NUM_RE = re.compile(
...
def _analyze_bullet(text: str) -> BulletAnalysis:
    ...
    has_metric = bool(
        _METRIC_RE.search(stripped)
        or _PERCENT_RE.search(stripped)
        or _TIME_REDUCTION_RE.search(stripped)
        or _INDIAN_NUM_RE.search(stripped)
    )
    has_number = bool(_NUMBER_RE.search(stripped))
    is_weak = any(stripped.lower().startswith(w) for w in _WEAK_OPENINGS)

    if not has_action and ner_available():
        try:
            nlp = _get_best_nlp()
            if nlp is not None:
                doc = nlp(stripped[:200])
                for token in doc:
                    if token.dep_ == "ROOT" and token.pos_ == "VERB":
                        lemma = token.lemma_.lower()
                        if lemma in _STRONG_ACTION_VERBS:
                            has_action = True
                            break
        except Exception:
            pass
