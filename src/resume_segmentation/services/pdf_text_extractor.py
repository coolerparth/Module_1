...
class PDFTextExtractor:
    def extract(self, pdf_path: str) -> tuple[list[str], list[str]]:
        try:
            ...
            raise RuntimeError(
                "No PDF text extractor succeeded. Install project dependencies in the virtual environment."
            ) from exc

    def _extract_with_pdfplumber(self, pdf_path: str) -> tuple[list[str], list[str]]:
        try:
            import pdfplumber
