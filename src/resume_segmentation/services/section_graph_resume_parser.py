...
class SectionGraphResumeParser:
    ...
    def _build_sections(self, pages) -> dict[str, list[str]]:
        ...
        for page in pages:
            page_font_sizes = [
                line.font_size for line in page.lines if line.font_size is not None
            ]
        ...
        if not headings:
            sections["header"] = [
                line.text.strip() for line, _ in flattened if line.text.strip()
            ]
            return sections
    ...
    def _detect_section(self, line: LineNode, median_font: float) -> str | None:
        ...
        font_boosted = line.font_size and line.font_size >= median_font * 1.05
        is_bold = line.is_bold
        normalized = self._normalize(line.text)
        is_short = len(normalized.split()) <= 4

        if font_boosted or is_bold or is_short:
            return detected
        return None
